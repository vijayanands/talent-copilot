...
def predict_productivity(jira_data, confluence_data, github_data):
    ...
    productivity_score = (total_jira_issues * 0.4 + 
...
def display_employee_stats(employee):
    ...
    # Jira Chart
    fig_jira = px.line(jira_data, x="date", y=["issues_created", "issues_resolved"], title="Jira Activity")
    st.plotly_chart(fig_jira)

    # Confluence Chart
    fig_confluence = px.line(confluence_data, x="date", y=["pages_created", "pages_edited"], title="Confluence Activity")
    st.plotly_chart(fig_confluence)

    # GitHub Chart
    fig_github = px.line(github_data, x="date", y=["commits", "pull_requests"], title="GitHub Activity")
