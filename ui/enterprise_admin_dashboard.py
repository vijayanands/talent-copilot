...
def edit_positions(ladder):
    ...
    with col2:
        new_level = st.number_input("New Position Level", min_value=1, max_value=len(positions) + 1,
...
def level_eligibility():
    ...
    ladders = get_all_ladders()
    selected_ladder = st.selectbox("Select Ladder", options=[ladder.name for ladder in ladders],
    ...
    if selected_ladder_obj:
        ...
        selected_position = st.selectbox("Select Position",
                                         options=[f"{selected_ladder_obj.prefix}{position['level']}: {position['name']}"
                                                  for position in positions],
                                         key="level_elig_position")

        selected_position_obj = next((position for position in positions if
                                      f"{selected_ladder_obj.prefix}{position['level']}: {position['name']}" == selected_position),
                                     None)
        ...
        if selected_position_obj:
            ...
            new_criteria["years_of_experience"] = st.number_input("Minimum years of experience",
                                                                  value=current_criteria.get("years_of_experience", 0),
                                                                  key="years_of_experience")
            new_criteria["projects_completed"] = st.number_input("Minimum projects completed",
                                                                 value=current_criteria.get("projects_completed", 0),
                                                                 key="projects_completed")
            new_criteria["leadership_experience"] = st.checkbox("Leadership experience required",
                                                                value=current_criteria.get("leadership_experience",
                                                                                           False),
