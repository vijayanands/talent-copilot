...
def main():
    ...
    st.markdown('<div class="title-container"><h1 class="title">PathForge Employee Assistant</h1></div>',
                unsafe_allow_html=True)
    ...
    if 'form_fields' not in st.session_state:
        st.session_state.form_fields = {
            ...
            'confluence_username': '', 'jira_username': '', 'atlassian_api_token': '',
            'github_username': '', 'github_token': '',
            'selected_range': 'Last year', 'start_date': None, 'end_date': None
        }
    ...
    with create_section("User Information", "#f0f0f0"):
        ...
        with st.container():
            ...
            """, "#e6f2ff")
            st.session_state.form_fields['first_name'] = st.text_input("First Name",
                                                                       value=st.session_state.form_fields['first_name'],
                                                                       key="first_name")
            st.session_state.form_fields['last_name'] = st.text_input("Last Name",
                                                                      value=st.session_state.form_fields['last_name'],
                                                                      key="last_name")
            st.session_state.form_fields['email'] = st.text_input("Email", value=st.session_state.form_fields['email'],
                                                                  key="email")
        ...
        with st.container():
            ...
            """, "#e6ffe6")
            st.session_state.form_fields['confluence_username'] = st.text_input("Confluence Username",
                                                                                value=st.session_state.form_fields[
                                                                                    'confluence_username'],
                                                                                key="confluence_username")
            st.session_state.form_fields['jira_username'] = st.text_input("Jira Username",
                                                                          value=st.session_state.form_fields[
                                                                              'jira_username'], key="jira_username")
            st.session_state.form_fields['atlassian_api_token'] = st.text_input("Atlassian API Token", type="password",
                                                                                value=st.session_state.form_fields[
                                                                                    'atlassian_api_token'],
        ...
        with st.container():
            ...
            """, "#fff2e6")
            st.session_state.form_fields['github_username'] = st.text_input("GitHub Username",
                                                                            value=st.session_state.form_fields[
                                                                                'github_username'],
                                                                            key="github_username")
            st.session_state.form_fields['github_token'] = st.text_input("GitHub Token", type="password",
                                                                         value=st.session_state.form_fields[
    ...
    with create_section("Time Range", "#f2e6ff"):
        ...
        st.session_state.form_fields['selected_range'] = st.selectbox("Select Time Range",
        ...
        if st.session_state.form_fields['selected_range'] == "Custom":
            ...
            with col1:
                st.session_state.form_fields['start_date'] = st.date_input("Start Date", value=default_start,
                                                                           min_value=two_years_ago, max_value=today,
                                                                           key="start_date")
            with col2:
                end_date = st.date_input("End Date", value=today, min_value=two_years_ago, max_value=today,
                                         key="end_date")
                if end_date > today:
            ...
            if st.session_state.date_error:
                st.markdown(f'<p class="error-message">{st.session_state.date_error}</p>', unsafe_allow_html=True)

            # Validate custom date range
            if st.session_state.form_fields['start_date'] and st.session_state.form_fields['end_date'] and \
                    st.session_state.form_fields['start_date'] > st.session_state.form_fields['end_date']:
            ...
            st.session_state.form_fields['start_date'], st.session_state.form_fields['end_date'] = time_range_options[
                st.session_state.form_fields['selected_range']]
            st.session_state.date_error = None

        # Ensure start date is not more than 2 years ago
        if st.session_state.form_fields['start_date']:
            st.session_state.form_fields['start_date'] = max(st.session_state.form_fields['start_date'], two_years_ago)

        if st.session_state.form_fields['start_date'] and st.session_state.form_fields[
            'end_date'] and not st.session_state.date_error:
            st.write(
    ...
    all_fields_filled = all(st.session_state.form_fields.values()) and not st.session_state.date_error
    ...
    if st.button("Generate Summary", disabled=button_disabled):
        data = {
            ...
            "atlassianInfo": {
                "confluenceUsername": st.session_state.form_fields['confluence_username'],
                "jiraUsername": st.session_state.form_fields['jira_username'],
                "atlassianApiToken": st.session_state.form_fields['atlassian_api_token']
            },
            "githubInfo": {
        ...
        st.warning("Please fill in all fields and resolve any date errors before generating the summary.")
